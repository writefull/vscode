...
async function processFileAttachment(
	...
	try {
		...
		if (range) {
			...
			logService.info(
				`[OverleafAgent] Attached file with selection: ${fileName} (lines ${range.startLineNumber}-${range.endLineNumber})`
			);
		}
		...
		logService.info(`[OverleafAgent] Attached file: ${fileName} (${content.value.byteLength} bytes)`);
		...
		logService.error('[OverleafAgent] Failed to read attached file:', error);
		return null;
	}
}
...
async function processDirectoryAttachment(
	...
	try {
		...
		if (stat.children) {
			...
			logService.info(`[OverleafAgent] Attached directory: ${dirName} (${stat.children.length} items)`);
		...
		logService.error('[OverleafAgent] Failed to read attached directory:', error);
		return null;
	}
}
...
export async function buildMessages(
	...
	if (request.locationData?.type === ChatAgentLocation.EditorInline) {
		...
		logService.info(
			`[OverleafAgent] Processing locationData for inline chat: ${documentUri.toString()}, selection: ${selectionRange.startLineNumber}-${selectionRange.endLineNumber}`
		);
		...
		try {
			...
			logService.info(
				`[OverleafAgent] Added inline chat context: ${fileName} with selection (${selectionRange.startLineNumber}-${selectionRange.endLineNumber})`
			);
		} catch (error) {
			logService.error('[OverleafAgent] Failed to process locationData:', error);
		}
	}
	...
	if (request.variables?.variables && request.variables.variables.length > 0) {
		logService.info(`[OverleafAgent] Processing ${request.variables.variables.length} attached context items`);
