...
export class WriteFileTool implements IToolImpl {
	static readonly TOOL_ID = 'overleaf_writeFile';

	static getToolData(): IToolData {
		return {
			id: WriteFileTool.TOOL_ID,
			displayName: localize('overleafTool.writeFile.displayName', 'Write File'),
			modelDescription: 'CRITICAL: Use ONLY for creating brand NEW files that do not exist yet. NEVER use this tool if the file already exists - it will overwrite and delete all existing content. For existing files, ALWAYS use overleaf_readFile first to read the current content, then use overleaf_editFile to make changes. This tool will reject attempts to write to existing files. This is a LaTeX editor, so by default: (1) If no file extension is specified in the path, default to .tex extension. (2) If no content is provided or content is empty, generate a basic LaTeX document template (\\documentclass{article}, \\begin{document}, etc.).',
					...
					return {
						content: [{
							...
							value: JSON.stringify({
								success: false,
								error: `File already exists: ${uri.fsPath}. Use overleaf_writeFile ONLY for creating new files. For existing files, you must: (1) First read the file with overleaf_readFile to see its current content, (2) Then use overleaf_editFile to make changes. This prevents accidentally deleting existing content.`,
								path: args.path,
								fileExists: true,
								suggestion: 'Use overleaf_readFile to read the file, then overleaf_editFile to modify it'
							})
						}]
					};
				}
