...
import { resolveFile, sanitizeInputFilePath } from './utils/fileUtils';
	...
	if (node.type === 'macro' && config.macros.secs.includes((node as Macro).content)) {
		...
		if (inputCommands.includes(macro.content)) {
			...
			const subFile = await resolveFile(
				[path.dirname(filePath), path.dirname(rootFilePath || ''), ...config.texDirs],
				arg0
			);
			if (subFile) {
				element = {
			...
			const subFile = await resolveFile(
				[arg0, path.join(path.dirname(rootFilePath || ''), arg0)],
				arg1
			);
			if (subFile) {
				element = {
...
async function parseRnwChildMacro(
	...
	while (true) {
		...
		const subFile = await resolveFile(
			[path.dirname(file), path.dirname(rootFile)],
			matchedPath
		);
		if (subFile) {
