...
export class FileSystemUtils {
	...
	static async exists(uri: vscode.Uri): Promise<boolean> {
		try {
			console.log('[FileSystemUtils] Checking if file exists:', uri.toString());
			await vscode.workspace.fs.stat(uri);
			console.log('[FileSystemUtils] File exists:', uri.toString());
			return true;
		} catch (error) {
			console.log('[FileSystemUtils] File does not exist:', uri.toString(), error);
			return false;
		}
	}
	...
	static async readFile(uri: vscode.Uri): Promise<string> {
		try {
			console.log('[FileSystemUtils] Reading file:', uri.toString());
			const data = await vscode.workspace.fs.readFile(uri);
			const content = new TextDecoder('utf-8').decode(data);
			console.log('[FileSystemUtils] Read', content.length, 'bytes from:', uri.toString());
			return content;
		} catch (error) {
			console.error('[FileSystemUtils] Failed to read file:', uri.toString(), error);
